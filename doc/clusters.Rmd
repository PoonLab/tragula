...
get.pairs <- function(nodes, cvr) {
  ...
  counts <- sapply(membership, length)
  
  pairs <- list()
  for (j in 0:max(counts)) {
    cand <- nodes[which(counts==j)]  # candidate nodes
    cmx <- combinat::combn2(cand)  # all pairs

    # filter pairs for those with exactly the same memberships
    same <- apply(cmx, 1, function(row) {
      all(membership[[row[1]]] == membership[[row[2]]])
    })
    
    pairs[[as.character(j)]] <- cmx[same, ]
  }
  pairs
}

omega <- function(nodes, c1, c2) {
  tj1 <- get.pairs(nodes, c1)
  tj2 <- get.pairs(nodes, c2)
  ...
  for (j in 0:max.K) {
    jj <- as.character(j)
    omega.u <- omega.u + length(intersect(
      apply(tj1[[jj]], 1, function(x) paste(sort(x), collapse=' ')),
      apply(tj2[[jj]], 1, function(x) paste(sort(x), collapse=' '))
      ))
    omega.e <- omega.e + nrow(tj1[[jj]]) * nrow(tj2[[jj]])
  }
}
