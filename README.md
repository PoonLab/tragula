...
is simply a wrapper around the multi-dimensional scaling function in R
(`cmdscale`):
...
idx <- match(row.names(wdist), fac$filename)
labels <- fac$initials[idx]

plot.wdist(wdist, labels=labels)
```

<img src="README_files/figure-gfm/unnamed-chunk-7-1.png" width="50%" />

We can also use R’s built-in hierarchical clustering methods:

``` r
hc <- hclust(as.dist(wdist))
